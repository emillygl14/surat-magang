...
export default function DataMahasiswa() {
  ...
  const [form, setForm] = useState({ nama: "", email: "", nim: "", password: "", noHp: "", programStudi: "" });
  ...
  const openModal = (isEdit, data = null) => {
    ...
    if (isEdit && data) {
      ...
      setForm({ nama: "", email: "", nim: "", password: "", noHp: "", programStudi: "" });
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Yakin ingin menghapus mahasiswa ini? Semua pengajuannya akan ikut terhapus.")) return;
    try {
  ...
  return (
    <>
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between shrink-0">
        <h1 className="text-xl font-bold text-gray-900">Kelola Data Mahasiswa</h1>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={(e) => { if (e.target === e.currentTarget) setModal({ show: false, isEdit: false, data: null }); }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-gray-900">{modal.isEdit ? "Edit Mahasiswa" : "Tambah Mahasiswa"}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={form.nama} onChange={(e) => setForm({ ...form, nama: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.nim} onChange={(e) => setForm({ ...form, nim: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Password {modal.isEdit && "(Isi jika ingin ganti)"}</label>
                <input type="password" required={!modal.isEdit} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div className="pt-2">
